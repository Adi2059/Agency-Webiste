...
const CofounderDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [leadsRes, docsRes] = await Promise.all([
        axios.get('http://localhost:5000/api/leads/all', { headers }),
        axios.get('http://localhost:5000/api/leads/docs/all', { headers })
      ]);
  ...
  const submitDealClose = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/leads/update-status/${dealToClose._id}`, {
        status: 'Closed',
  ...
  const submitEditPayment = async (e) => {
    ...
    setIsSubmitting(true);
    try {
      await axios.put(`http://localhost:5000/api/leads/update-status/${dealToEdit._id}`, {
  ...
  const submitProjectUpdate = async (e) => {
    ...
    setIsSubmitting(true);
    try {
      await axios.put(`http://localhost:5000/api/leads/update-status/${projectToEdit._id}`, {
  ...
  const handleRemoveDeal = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/leads/delete/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('afterus_token')}` }});
  ...
  const handlePdfUploadSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5000/api/leads/docs/upload', uploadData, {
  ...
  const handleBulkCSVUpload = (e) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        const token = localStorage.getItem('afterus_token');
        const res = await axios.post('http://localhost:5000/api/leads/bulk-add', { leads: leadsArray }, { headers: { Authorization: `Bearer ${token}` } });
