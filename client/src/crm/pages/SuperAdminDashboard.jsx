...
const SuperAdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [dashRes, teamRes, allLeadsRes, docsRes] = await Promise.all([
        axios.get('http://localhost:5000/api/leads/dashboard-data', { headers }),
        axios.get('http://localhost:5000/api/leads/team', { headers }),
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await Promise.all(selectedLeads.map(id => 
        axios.delete(`http://localhost:5000/api/leads/delete/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      ));
  ...
  const handleBulkCSVUpload = (e) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        const token = localStorage.getItem('afterus_token');
        const res = await axios.post('http://localhost:5000/api/leads/bulk-add', 
  ...
  const handleTeamSubmit = async (e) => {
    ...
    try {
      if (isEditingTeam) {
       await axios.put(`http://localhost:5000/api/leads/team/edit/${teamFormData.id}`, {
        ...
        await axios.post('http://localhost:5000/api/leads/team/add', teamFormData, { headers });
      }
